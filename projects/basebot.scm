...
(time-to-height 1 2 3)
(time-to-height 4 2 1)
(time-to-height 0 2 0)
(time-to-height 2 0 0)
(time-to-height 0 0 0)
...
(define travel-distance-simple
  (lambda (elevation velocity angle)
    (let ((angle-rad (degree2radian angle)))
      (* velocity 
        (cos angle-rad) 
        (time-to-impact (y-vector velocity angle) 
                        elevation)))))

(travel-distance-simple 2 4 25)
...
(time-to-impact 1 (y-vector 45 0))
(time-to-impact 1 (y-vector 45 90))
(time-to-impact 1 (y-vector 45 45))
...
(define find-best-angle
  (lambda (velocity elevation)
    (define find-max-angle
      (lambda (best elevation velocity angle)
        (let ((new (travel-distance-simple elevation velocity angle)))
          (newline)
          (display angle)
          (display " - ")
          (display new)
    ...
    (define iter
      (lambda (angle best)
        (if (< 90 angle)
          (car best)
          (iter (+ angle alpha-increment)
                (find-max-angle best elevation velocity angle))))) 

    (iter 0 '(0 0))))

(find-best-angle 10 10)
(find-best-angle 45 45)
(find-best-angle 0 0)
